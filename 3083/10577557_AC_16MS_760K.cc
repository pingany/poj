...
typedef Point FindNextFunc(Point p, Direction& dir);
#define valid(ret) (ret.x >= 0 && ret.x < w \
	&& ret.y >= 0 && ret.y <h && mazz[ret.y][ret.x] != '#')
Point getMost(Point p, Direction&currentDir, bool leftMost)
{
...
{
	int step = 0;
	Point next = getMost(p, dir, leftMost);
	while(mazz[next.y][next.x] != 'E')
	{
		step++;
		next = getMost(next, dir, leftMost);
	}
	cout << step+2 << " ";
}
...
void shortpath(Point sp)
{
	...
	for(int i = 0; i < allPointIndex; i++)
	{
		for(int j = 0; j < 4; j++)
		{
			Point next = allPoints[i].p + nextPs[j];
