...
bool testcase()
{
	...
	vector<Stuff>::iterator it = v.begin() + 1;
	for(; it < v.end();)
	{
		char c = it->c;
		switch(c)
		...
		case ')':
			{
				...
				else
				{
					...
					if(left < right)
					{
						...
						result.push_back(Stuff(opps(againstc), (it-1)->offset+1));
						it = v.erase(it-1);
					}
					else
					{
	...
	{
		ASSERT(it->c == '[' || it->c == '(');
		result.push_back(Stuff(opps(it->c), it->offset+1));
	}
	for(Stuffs::iterator it = result.begin(); it < result.end(); it++)
	{
		s.insert(it->offset, 1, it->c);
	}
	cout << s <<"\n";
	return true;
}
