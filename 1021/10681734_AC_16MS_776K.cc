...
bool subg_cmp(const SubGraph&g1, const SubGraph&g2)
{
	return g1.maxWH != g2.maxWH ? g1.maxWH > g2.maxWH : g1.minWH > g2.minWH;
}
...
bool solve()
{
	...
	if(subsNum[0] != subsNum[1])
		return false;
	int n = subsNum[0];
	sort(subs[0], subs[0]+n, subg_cmp);
	sort(subs[1], subs[1]+n, subg_cmp);
	forn(i, 0, n)
	{
		SubGraph &s = subs[0][i];
		ASSERT(!s.matched);
