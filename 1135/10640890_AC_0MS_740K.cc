...
int solve()
{
	...
	forn(i, 0, n)
	{
		 ...
		 it != node.adjs.end(); ++it)
		{
			...
			int big = node.value;
			int small = adj.value;
			double edgeValue = (e.weight + big + small)/2.0;
			if(edgeValue > maxEdgeValue + 0.01)
			{
	...
	if(maxEdgeValue > (double)maxValue + 0.01)
	{
		if(edgeX > edgeY)
			swap(edgeX, edgeY);
	...
	{
		printf(out[0], (double)maxValue, maxNodeIndex+1);
	}
