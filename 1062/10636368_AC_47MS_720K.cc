...
bool visiting[MAXN];
int getMinValue(int startNode, int maxValue, int minLevel, int maxLevel)
{
	if(startNode == 1)
		return 0;
	int ret = maxValue;
	...
	for (NodeAdjs::iterator it = node.adjs.begin();
		 it != node.adjs.end(); ++it)
		{
			...
			if(!visiting[e.nodeIndex])
			{
				...
				if(minL > maxL)
					continue;
				ret = min(ret, e.weight + getMinValue(adj.index, maxValue-e.weight,
					minL, maxL));
			}
		}
...
bool testcase()
{
	...
	cout << getMinValue(0, nodes[0]->adjs[0].weight, 0, MAX_INT);
	return true;
}
