...
int dp1(int startNode, int childIndex, int k, Type type, int &ret);
inline int dp(int startNode, int childIndex, int k, Type type)
{
	if(childIndex < 0 || k == 0)
		return 0;
	int &ret = dpRecord[startNode][childIndex][k][type];
	if(ret != INVALID_DP_RECORD)
		return ret;
	return dp1(startNode, childIndex, k, type, ret);
}
...
inline int get(int startNode, int k, Type type)
{
	...
	int r = apples[startNode] + dp(startNode, nodes[startNode].size()-1, k, type);
...
int dp1(int startNode, int childIndex, int k, Type type, int &ret)
{
	...
	ASSERT(k > 0 && k <= K);
	ASSERT(childIndex >=0 && childIndex < nodes[startNode].size());

	
	ret = 0;	
	int theNb = nodes[startNode][childIndex];
	if (visited[theNb])
	{
		ret = max(ret, dp(startNode, childIndex-1, k, type));
	}
	else
	{
		// x = 0, don't access theNb
		ret = max(ret, 
				+ dp(startNode, childIndex-1, k, type));
		if (type == BACK)
		{
			...
			for(int x = 0; x <=k-2; x+=2)
			{
				ret = max(ret, get(theNb, x, BACK) \
					+ dp(startNode, childIndex-1, k-2-x, BACK));
			}
		}
		else
		{
			...
			for(int x = 0; x <=k-2; x+=2)
			{
				...
				lastValue = r;
				ret = max(ret,  r\
					+ dp(startNode, childIndex-1, k-2-x, NO_BACK));
			}
			lastValue = -1;
			...
			for(int x = 0; x <=k-1; x+=1)
			{
				...
				lastValue = r;
				ret = max(ret, r \
					+ dp(startNode, childIndex-1, k-1-x, BACK));
			}
		}
